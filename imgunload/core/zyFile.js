...
var ZYFILE = {
		...
		funUploadFile : function(file){
			var self = this;  // 在each中this指向没个v  所以先将this保留
			var formdata = new FormData();//使用formData实现图片上传
			formdata.append("fileList", file);//给formdata添加文件的信息	    
			...
			xhr.setRequestHeader("X_FILENAME", encodeURI(encodeURI(file.name)));//支持中文
			//xhr.setRequestHeader("X_FILENAME", file.name);
			xhr.send(formdata);//上传图片的数据
		},
		// 返回需要上传的文件
